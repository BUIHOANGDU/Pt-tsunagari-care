(function () {
  ...
  async function testFallAlert() {
    ...
    try {
      const docRef = await createTestFallAlert();
      const message = `Test fall alert sent: ${docRef.id}`;
      addLog(message);
      console.log("FallCamera:", message);
    } catch (error) {
      const message = `Test fall alert failed: ${error.message}`;
      addLog(message);
      console.warn("FallCamera:", message, error);
    } finally {
  ...
  testFallAlertButton.addEventListener("click", testFallAlert);
